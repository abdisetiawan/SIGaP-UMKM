...
var OLLatLonPicker = function () {
    ...
    var b = function (a, b) {
            (function (c) {
                c.ajax({
                    url: "http://maps.googleapis.com/maps/api/geocode/json?latlng=" + encodeURIComponent(a) + "," + encodeURIComponent(b) + "&sensor=false",
                    dataType: "json"
                }).done(function (a) {
                    "OK" == a.status ? f(a.results[0].formatted_address) : f("")
                })
            })(jQuery)
        },
        c = function (c) {
            (function (f) {
                f.ajax({
                    url: "http://maps.googleapis.com/maps/api/geocode/json?sensor=false&address=" + encodeURIComponent(c),
                    dataType: "json"
                }).done(function (c) {
                    if ("OK" == c.status) {
                        lat = c.results[0].geometry.location.lat, lng = c.results[0].geometry.location.lng;
                        var f = new OpenLayers.LonLat(lng, lat).transform(a.vars.latLongProj, a.vars.map.getProjectionObject());
                        g(f.lat, f.lon, 10), e(f.lat, f.lon, a.vars.map.getZoom()), b(lat, lng), d(lat, lng)
                    } else alert("No position found")
                })
            })(jQuery)
        },
        d = function (b, c) {
            (function (d) {
                d.ajax({
                    url: "http://maps.googleapis.com/maps/api/elevation/json?sensor=false&locations=" + b + "," + c,
                    dataType: "json"
                }).done(function (b) {
                    "OK" == b.status && (elevation = b.results[0].elevation, d(a.vars.form).find(".gllpElevation").val(elevation.toFixed(3)), k())
                })
            })(jQuery)
        },
        e = function (b, c, d) {
        ...
        g = function (b, c, d) {
            ...
            var e = new OpenLayers.LonLat(c, b),
                f = new OpenLayers.Size(21, 25),
                g = new OpenLayers.Pixel(-(f.w / 2), -f.h),
                h = new OpenLayers.Icon("images/marker.png", f, g);
